...
let userOptions = {
    ...
    userDn: `uid={{username}},ou=SpecialUsers,dc=entp,dc=tgc`,
...
describe('Test ldap authenticate', () => {
    ...
    test('invalid username/password result', () => {
        return request(app).post("/login")
            .type('json')
            .send({ username: 'radar', password: 'Test12' })
            .then(response => {
                expect(response.statusCode).toBe(401)
                expect(response.body.success).toBeFalsy()
                expect(response.body.message).toEqual("Invalid Credentials")
                expect(response.body.user).toBeUndefined()
                user = User.find(u => { return u.username === "radar" })
    ...
    test('correct username/password result', () => {
        return request(app).post("/login")
            .type('json')
            .send({ username: 'radar', password: 'Test1234' })
            .then(response => {
                expect(response.statusCode).toBe(200)
                expect(response.body.success).toBeTruthy()
                expect(response.body.message).toBe('authentication succeeded')
                expect(response.body.user.uid).toBe('radar')
                user = User.find(u => { return u.uid === "radar" })
